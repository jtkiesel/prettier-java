...
export function printIgnored(node: JavaNode, options: JavaParserOptions) {
  const {
    [Symbol.for("comments")]: comments,
    [Symbol.for("printedComments")]: printedComments
  } = options as unknown as Record<keyof any, IToken[] | Set<IToken>>;
  const start = locStartWithComments(node);
  const end = locEndWithComments(node);
  for (const comment of comments) {
    if (options.locStart(comment) >= start && options.locEnd(comment) <= end) {
      (printedComments as Set<IToken>).add(comment);
    }
  }
