...
export function isFullyBetweenPrettierIgnore(path: JavaPath) {
...
export function willPrintOwnComments(path: JavaPath): boolean {
...
function printComment(path: JavaPath) {
  const comment = path.node as unknown as JavaComment;
  comment.printed = true;
  return printer.printComment(path);
}

function printLeadingComment(path: JavaPath) {
  const comment = path.node;
  const parts: Doc[] = [printComment(path)];
...
function printTrailingComment(
  ...
  const comment = path.node;
  const printed = printComment(path);
...
function printLeadingComments(path: JavaPath) {
  ...
  path.each(path => {
    ...
    if (comment.printed || !comment.leading) {
      return;
    }

    docs.push(printLeadingComment(path as unknown as JavaPath));
  }, "comments");

  return docs;
}

function printTrailingComments(path: JavaPath) {
  if (!hasChild(path, "comments")) {
    return [];
  }
  const docs: Doc[] = [];
  ...
  path.each(path => {
    const { node: comment } = path;
    if (comment.printed || !comment.trailing) {
      return;
    }

    printedTrailingComment = printTrailingComment(
      path as unknown as JavaPath,
...
export function printComments(path: JavaPath, doc: Doc) {
  const leading = printLeadingComments(path);
  const trailing = printTrailingComments(path);
  return leading || trailing ? [leading, doc, trailing] : doc;
}
